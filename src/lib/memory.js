...
/**
 * Extract durable facts from a single user message.
 * Returns an array of { fact, category } objects.
 */
export async function extractFactsFromMessage(userMessage, assistantReply) {
  if (!userMessage?.trim()) return []

  const prompt = `The user just said: "${userMessage.slice(0, 2000)}"
${assistantReply ? `The advisor replied: "${assistantReply.slice(0, 1000)}"` : ''}

Extract any durable facts from the user's message that should be remembered for future conversations (income changes, employment details, goals, debts, family events, risk preferences, etc.).
