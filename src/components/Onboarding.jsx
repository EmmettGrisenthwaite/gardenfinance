...
const STEPS = [
  {
    ...
    sub: 'This takes about 2 minutes and helps your advisor give you real advice — not generic tips.',
    type: 'intro',
  },
  {
    id: 'basics',
  ...
  },
  {
    ...
    sub: "Your advisor will focus conversations around this — you can change it anytime.",
    type: 'single',
    field: 'primary_goal',
    options: [
...
export default function Onboarding({ onClose }) {
  ...
  // Skip intro slide when opened manually (editing existing profile)
  const startStep = onClose ? 1 : 0
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm sm:p-4">
      ...
      >
          ...
          <div className="flex items-center gap-1.5">
            {STEPS.map((_, i) => (
              <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${
                i < step ? 'bg-white w-4' : i === step ? 'bg-white w-6' : 'bg-white/30 w-3'
              }`} />
            ))}
          </div>
          <div className="text-white/70 text-xs mt-2">Step {step + 1} of {STEPS.length}</div>
        </div>
            ...
            <motion.div key={step}
              ...
              {current.type === 'intro' && (
                <div className="space-y-3 mt-2">
                  {['Age → compounding math gets real', '401k match → free money check', 'Investment status → no repeat advice', 'Health insurance → critical gap check', 'Your #1 goal → focused conversations'].map((item, i) => (
        ...
        <div className="px-6 pb-5 flex items-center justify-between">
          ...
          {/* Show Next/Finish button for non-auto-advance steps */}
          {(current.type === 'intro' || current.type === 'age' || current.type === 'multi') && (
            isLast ? (
          ...
          {/* For single-select last step, show finish separately */}
