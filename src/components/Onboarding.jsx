    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-2 sm:flex sm:items-stretch">
        <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Visa, student loan"
          ...
          className="w-full min-w-0 px-3 py-2.5 rounded-xl border-2 border-white/15 bg-white/[0.075] text-white text-sm focus:outline-none focus:border-emerald-500 transition-colors sm:flex-1" />
        <div className="flex items-center bg-white/[0.075] border-2 border-white/15 rounded-xl px-2.5 focus-within:border-emerald-500 transition-colors sm:w-24">
        ...
        <button type="button" onClick={add} disabled={!name.trim() || !(parseFloat(bal) > 0)}
          className="min-h-11 px-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-white/10 disabled:text-white/30 text-white text-sm font-semibold transition-colors">
          Add
        </button>
      </div>
